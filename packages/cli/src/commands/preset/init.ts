...
export type PlatformDetection =
  | { insidePlatformDir: true; platform: PlatformId }
  | { insidePlatformDir: false; platforms: DetectedPlatform[] };
...
/**
 * Detect platform context for a directory.
 *
 * Returns either:
 * - { insidePlatformDir: true, platform } if directory itself is a platform dir
 * - { insidePlatformDir: false, platforms } with detected platform dirs inside
 */
export async function detectPlatformContext(
  directory: string
): Promise<PlatformDetection> {
  const platform = getPlatformFromDir(basename(directory));

  if (platform) {
    return { insidePlatformDir: true, platform };
  }

  const platforms = await detectPlatforms(directory);
  return { insidePlatformDir: false, platforms };
}
...
export async function initPreset(options: InitOptions): Promise<InitResult> {
  ...
  if (!platformInput) {
    throw new Error(
      "Cannot determine platform. Specify --platform or run from a platform directory."
    );
  }
